...
| 2026-06-01 | 1.0.1 | Change Content | Nam Jae-Min |
</h2>
...
<img width="1032" height="781" alt="image" src="image/UseCase/UseCase diagram.png" />
<br>
<br><br><br><br>
...
| 1 | 사용자는 유형에 따라 [회원가입(소비자용)] 버튼 또는 [사업자 등록(판매자용)] 버튼을 클릭한다. 
| 2 |  시스템은 이름, 아이디, 비밀번호 입력화면을 출력한다. 판매자 유형일 경우 UC #13(Add Market) 입려도 필요하다. |
| 3 | 사용자가 필수정보(이름,아이디와 비밀번호) 입력한다.    |
| 4 | 시스템은 입력받은 내용이 데이터베이스에 존재하는지 검사한다. |
| 5 | 시스템은 일치하는 정보가 없으면 계정정보를 데이터베이스에 저장한다. |
...
||2a2. 로그인화면으로 돌아간다.|
| 3 | 3a. 비밀번호를 7자리 미만으로 입력하는 경우.|
||3a1. "비밀번호는 7자리 이상이어야합니다." 메시지를 출력한다.|
||3b. 필수 정보(이름,아이디,비밀번호)이 누락된 경우.|
||3b1. "모든 필수 정보를 다 입력해주세요" 메시지를 출력한다.|
||3c. 사용자가 X(닫기) 버튼을 누르는 경우.|
||3c1. 진행중인 회원가입 프로세스를 중단하고, 로그인화면으로 이동한다. |
| 4 | 4a.  검사 시, 입력한 정보에서 아이디가 이미 존재하는경우 |
||4a1. "이미 존재하는 아이디입니다" 메시지를 출력한다.|
| 5 | 5a. 네트워크 오류나 통신장애로 인해 데이터베이스에 저장할 수 없는경우.|
...
| 1 | 시스템은 외부API(카카오 우편번호 API)를 불러와 사용자의 주소 입력할 수 있게한다.  |
| 2 | 사용자가 도로명 주소를 검색하여 선택하고, 상세 주소(동, 호수, 층수 등)를 입력한 뒤 저장 버튼을 선택한다. |
| 3 | 시스템은 데이터베이스에 입력한 정보를 저장한다. |
| 4 | 정상적으로 저장되면 이 Usecase는 끝난다.|
| **EXTENSION SCENARIO** | |
| **Step** | **Branching Action** |
| 1 | 1a. 네트워크 오류나 통신장애로 인해 API와 통신이 안되는 경우. |
||1a1.  "통신이 원활하지 않습니다. 나중에 다시 시도하세요." 메시지를 출력한다.|
||1a2. 설정화면으로 돌아간다. |
| 2 | 2a. 비정상적인 입력하는 경우. |
...
||1b1. "지역 설정이 필요합니다. 주변 할인 정보를 확인하려면 먼저 지역 설정을 해주세요."|
||1b2. 소비자가 [설정하기]를 선택하면 UC#4(Register Region)을 수행한다.|
...
| 2 | 시스템은 현재 식료품 리스트로 외부 데이터베이스(조리식품의 DB)에 있는 레시피에서 재료 및 수량이 모두 해당하는지 계산한다.|
| 3 | 레시피의 상세정보(추가 구매필요한 식료품,난이도,소요시간,등)를 출력한다.|
...
||1a2. 메인 화면으로 돌아간다.|
||1b. 등록된 식료품이 없는 경우.|
||1b1. "등록된 식료품이 없습니다" 메시지를 출력한다. | 
| 2 |2a. 네트워크 오류나 통신장애로 인해 계산 및 데이터베이스와의 통신이 원활하지 않는 경우. |
||2a1. "통신이 원활하지 않습니다. 나중에 다시 시도하세요." 메시지를 출력한다.|
||2a2. 메인 화면으로 돌아간다.
| 3 | 3a. 네트워크 오류나 통신장애로 인해 화면 출력이 되지않는 경우. |
||3a1. "통신이 원활하지 않습니다. 나중에 다시 시도하세요." 메시지를 출력한다.|
...
||4b. 유효하지않는 사업장 번호인 경우. |
||4b1. "유효하지않는 사업장 번호입니다."메시지를 출력한다.|
...
||4b1. "통신이 원활하지 않습니다. 나중에 다시 시도하세요." 메시지를 출력한다.|
||4c. 이미 동일한 식료품에 대한 정보가 등록되어 있는 경우.|
||4c1. "이 식료품에 대한 할인이 등록되어 있습니다" 메시지를 출력한다. |
| **RELATED INFORMATION** | |
| Performance | <= 3 second |
| Frequency | Variable |
| Concurrency | No Limits |
...
3-2. User
<br>
이 서비스를 사용하는 사용자로, Consumer와 Seller의 부모 클래스입니다. 시스템 이용자의 공통속성(ID,PW,이름,역할,알림설정)을 정의합니다.
<br><br>
3-3. Consumer
<br>
소비자 사용자 클래스입니다. 자신의 식료품 저장소를 관리하고, 현재 식료품으로 만들 수 있는 레시피들을 조회하고 자세한 정보도 알 수 있습니다.
<br><br>
3-4. Seller
<br>
판매자 사용자 클래스입니다. 회원가입 시 자신의 사업장의 정보를 즉시 등록하며, 자신의 사업장의 등록된 식료품 재고가 남은 상품에 대한 할인 정보를 게시하여 홍보할 수 있습니다.
<br><br>
3-5. Food
<br>
...
<img width="382" height="845" alt="image" src="image/Prototype/Home.png" />
</div>
<br>
...
<br>
<div style="display: flex; justify-content: center; gap: 10px;">
<img width="377" height="831" alt="image" src="image/Prototype/Login.png" />
</div>
<br>
...
<br>
<div style="display: flex; justify-content: center; gap: 10px;">
<img width="377" height="841" alt="image" src="image/Prototype/Register.png" />
</div>
<br>
...
<br>
<div style="display: flex; justify-content: center; gap: 10px;">
<img width="376" height="844" alt="image" src="image/Prototype/Register Seller.png" />
</div>
<br>
...
<br>
<div style="display: flex; justify-content: center; gap: 10px;">
<img width="384" height="843" alt="image" src="image/Prototype/Main Consumer.png" />
<img width="395" height="847" alt="image" src="image/Prototype/Main Seller.png" />
</div>
<br>
...
<br>
<div style="display: flex; justify-content: center; gap: 10px;">
<img width="388" height="849" alt="image" src="image/Prototype/Setting.png" />
</div>
<br>
...
<br>
<div style="display: flex; justify-content: center; gap: 10px;">
<img width="375" height="851" alt="image" src="image/Prototype/Add Food.png" />
</div>
<br>
...
<br>
<div style="display: flex; justify-content: center; gap: 10px;">
<img width="375" height="850" alt="image" src="image/Prototype/Check Expiry Date.png" />
</div>
<br>
...
<br>
<div style="display: flex; justify-content: center; gap: 10px;">
<img width="384" height="845" alt="image" src="image/Prototype/Search Food.png" />
</div>
<br>
...
<br>
<div style="display: flex; justify-content: center; gap: 10px;">
<img width="366" height="855" alt="image" src="image/Prototype/Update Food1.png" />
<img width="378" height="842" alt="image" src="image/Prototype/Update Food2.png" />
</div>
<br>
...
<br>
<div style="display: flex; justify-content: center; gap: 10px;">
<img width="368" height="850" alt="image" src="image/Prototype/Delete Food1.png" />
<img width="388" height="845" alt="image" src="image/Prototype/Delete Food2.png" />
</div>
<br>
...
<br>
<div style="display: flex; justify-content: center; gap: 10px;">
<img width="385" height="852" alt="image" src="image/Prototype/Check Sale.png" />
<img width="393" height="850" alt="image" src="image/Prototype/Check Sale detail.png" />
</div>
<br>
...
<br>
<div style="display: flex; justify-content: center; gap: 10px;">
<img width="386" height="853" alt="image" src="image/Prototype/Check Sale Seller.png" />
</div>
<br>
...
<br>
<div style="display: flex; justify-content: center; gap: 10px;">
<img width="387" height="847" alt="image" src="image/Prototype/Add Sale Information.png" />
</div>
<br>
...
<br>
<div style="display: flex; justify-content: center; gap: 10px;">
<img width="396" height="857" alt="image" src="image/Prototype/Delete Sale Information1.png" />
<img width="392" height="853" alt="image" src="image/Prototype/Delete Sale Information2.png" />
</div>
<br>
