...
1) Summary
<br>
사람들은 음식을 마트에서 장을 보고 보통 식료품에 식료품을 저장한다. 하지만 바쁜 일상생활로 인해 식료품에 어떤 음식이 있는지 소비기한은 언제까지인지 음식이 상하였는지 등등의 정보를 매번 확인할 수 없을 뿐만아니라 까먹는다. 그래서 현재 식료품에 존재하는 음식인데도 불구하고 있는지 까먹고 다시 또 산다거나, 소비기한이 지난지 모르고 먹어서 식중독이 걸리는 경우, 상한음식으로 인해서 식료품에서 냄새가 나는 불편함을 종종 겪는다. 그리고 마트나 편의점의 판매자의 경우도 소비기한이 얼마남지 않은 물품의 처리의 어려움과 할인을 했음에도 고객이 할인의 정보를 모르고 찾아오지않는 경우로 인해 마트나 편의점의 수익이 줄어드는 현상이 나타난다. 이러한 소비자와 판매자의 어려움을 해결하기위해서 Food Manager라는 시스템을 고안했다. 
<br><br>
...
| 5 | 사용자는 바로 자동으로 로그인되고, 판매자의 경우 사업장 등록화면으로 이동한다. |
| 6 | 정상적으로 정보가 등록되면 이 usecase는 끝난다.|
| **EXTENSION SCENARIO** | |
| **Step** | **Branching Action** | 
| 1 | 1a.  네트워크 오류나 통신장애로 인해 화면 출력이 되지않는 경우. |
||1a1. "통신이 원활하지 않습니다. 나중에 다시 시도 하세요." 메시지를 출력한다.|
||1a2. 로그인화면으로 돌아간다.|
...
||4a1.  "통신이 원활하지 않습니다. 나중에 다시 시도 하세요." 메시지를 출력한다.|
| **RELATED INFORMATION** | |
...
||3d1.  "통신이 원활하지 않습니다. 나중에 다시 시도 하세요." 메시지를 출력한다.|
||3e. [회원가입]버튼을 누르는 경우.|
...
||3a1. "통신이 원활하지 않습니다. 나중에 다시 시도 하세요." 메시지를 출력한다.|
| **RELATED INFORMATION** | |
| Performance | <= 3 second |
| Frequency | Variable |
| Concurrency | No Limits |
...
||5a1. "통신이 원활하지 않습니다. 나중에 다시 시도 하세요." 메시지를 출력한다.|
| **RELATED INFORMATION** | |
| Performance | <= 3 second |
| Frequency | Variable |
| Concurrency | No Limits |
...
||2a1. 메인 화면으로 돌아간다.|
| 3 |3a. 소비기한 없음을 누른 경우.|
||3a1. 시스템은 소비기한 입력란을 비활성화하고 저장한다.|
| 4 |4a. 항목이 누락되어 있는 경우.|
...
||1a1. "등록된 식료품이 없습니다. 식품을 먼저 추가해주세요." 메시지를 출력한다.|
...
| 4 | 시스템은 변경하려는 항목의 기존정보을 디폴트로 가지는 UC#6(Add Food)의 입력화면과 같은 화면을 출력한다.|
| 5 | 사용자가 변경된 정보를 입력한다.|
...
| Trigger | 판매자가 회원가입 화면에서 사업장 등록(판매자용) 선택할때 |
...
||3e. 취소버튼을 누르는 경우.|
||3e1. 할인 정보 등록 프로세스는 중단되고, 할인관리 화면으로 돌아간다. |
...
| **MAIN SUCCESS SCENARIO** | |
| **Step** | **Action** |
| S | 판매자가 할인 관리 화면에서 할인정보 삭제를 선택한다. |
...
||2b2. 할인 관리 화면으로 돌아간다.|
| 3 | 3a. 판매자가 취소버튼을 누르는 경우. |
||3a1. 할인정보 삭제 프로세스는 중단되고, 할인관리 화면으로 돌아간다. |
| 4 | 4a. 네트워크 오류나 통신장애로 인해 화면 출력이 되지않는 경우. |
||4a1. "통신이 원활하지 않습니다. 나중에 다시 시도하세요." 메시지를 출력한다.|
||4a2. 할인 관리 화면으로 돌아간다.|
